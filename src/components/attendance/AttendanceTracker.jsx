...
const AttendanceTracker = ({
  ...
  //     // Call existing face/verify endpoint (port 8111)
  //     const response = await fetch('https://192.168.48.201:8111/api/face/verify', {
  //       method: 'POST',
  //       body: formData,
  //     });
 ...
 const performFaceAuthCheck = useCallback(async () => {
  ...
  try {
    ...
    // FIXED: Convert to base64 string (not blob)
    const imageBase64 = canvas.toDataURL('image/jpeg', 0.8);

    console.log(`🔐 Checking face authentication for user ${userId}...`);

    // FIXED: Use faceAuthAPI service with correct format
    const authResult = await faceAuthAPI.verifyFace(userId, imageBase64);
    ...
    console.error('❌ Face auth check error:', error);
    // Don't increment counter on technical errors
  } finally {
    if (mountedRef.current) {
      setAuthCheckInProgress(false);
    }

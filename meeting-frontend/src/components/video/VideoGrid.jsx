...
const filteredParticipants = useMemo(() => {
  console.log('🎥 VideoGrid - Filtering participants - Raw data:', {
    allParticipants: participants.map(p => ({
      ...
      Status: p.Status,
      LiveKit_Connected: p.LiveKit_Connected
    })),
    isCurrentUserHost,
  ...
  // 🔥 CRITICAL FIX: Common filter function for active participants
  const isActiveParticipant = (p) => {
    ...
    // Filter out offline/removed participants
    if (p.Status === 'offline' || p.Status === 'removed' || p.Status === 'left') {
      return false;
    }
    
    // CRITICAL: Must have valid user_id
    if (!p.user_id) {
      return false;
    }
  ...
  if (isCurrentUserHost) {
    // 🔥 HOST VIEW: Show ALL non-host participants (students + co-hosts)
    const nonHostParticipants = participants.filter(p => {
      if (!isActiveParticipant(p)) return false;
      
      // CRITICAL: Host should NOT see themselves in the grid
      if (p.isLocal || p.user_id === currentUser?.id) {
        return false;
      }
      
      // CRITICAL: Exclude other hosts, but INCLUDE co-hosts and regular participants
    ...
    // 🔥 PARTICIPANT VIEW: Show themselves + ALL hosts/co-hosts
    
    // Step 1: Find local participant (themselves)
    const localParticipant = participants.find(p => {
      if (!isActiveParticipant(p)) return false;
      return (p.isLocal || p.user_id === currentUser?.id);
    });
    
    // Step 2: Find ALL hosts and co-hosts
    const hostsAndCoHosts = participants.filter(p => {
      if (!isActiveParticipant(p)) return false;
      
      // CRITICAL: Don't show themselves again
      if (p.isLocal || p.user_id === currentUser?.id) {
        return false;
      }
      
      // CRITICAL: Include ALL hosts (role='host' OR isHost flag) AND co-hosts
      const isHostUser = (p.role === 'host' || p.isHost === true);
      const isCoHostUser = (p.isCoHost === true);
      
      // MUST be connected to LiveKit or have a stream
      const isConnected = p.LiveKit_Connected || p.Has_Stream || p.Status === 'live';
      
      console.log(`🔍 Checking participant ${p.displayName}:`, {
        user_id: p.user_id,
        role: p.role,
        isHost: p.isHost,
        isCoHost: p.isCoHost,
        ...
        isCoHostUser,
        isConnected,
        willShow: (isHostUser || isCoHostUser) && isConnected
      });
      
      return (isHostUser || isCoHostUser) && isConnected;
    });
    
    console.log('🎥 VideoGrid: PARTICIPANT VIEW - Details:', {
    ...
    // CRITICAL: Combine local + hosts/co-hosts
    const result = [
      ...(localParticipant ? [localParticipant] : []),
      ...hostsAndCoHosts
    ];
    
    console.log('🎥 VideoGrid: PARTICIPANT VIEW - Final result:', {
      count: result.length,

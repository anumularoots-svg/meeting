  ...
  async (meetingId) => {
    ...
    try {
      ...
      if (foundMeeting) {
        ...
        // CRITICAL FIX: Enhanced recurrence data parsing with proper date handling
        let recurrenceData = {
        ...
        // CRITICAL FIX: Parse end dates from multiple sources
        ...
        // Priority 1: Use recurrence_end_date if it exists (when user specifically set an end date for recurrence)
        ...
        // Priority 2: For recurring meetings, if no recurrence_end_date but end_date exists,
        // check if end_date is significantly different from meeting end time (indicating it's a series end)
          ...
          try {
            ...
            // If end_date is more than 1 day after the meeting end time, it's likely a series end date
            ...
            if (daysDiff > 1) {
              // This looks like a series end date, use it as recurrence end date
        ...
        const newMeetingData = {
          ...
          recurrence: recurrenceData, // Use the properly parsed recurrence data with end date
          reminders: {
...
useEffect(() => {
  ...
  // COMPLETE REPLACEMENT for your handleScheduleMeeting function in ScheduleMeeting.jsx
const handleScheduleMeeting = async () => {
  ...
  try {
    ...
    // CRITICAL FIX: Proper timezone handling to prevent time conversion issues
    ...
    // FIXED: Create datetime in the selected timezone without conversion
    const startDateTime = new Date(startDate);
    
    if (startTime instanceof Date) {
      // Use the time from the time picker, but keep it in local interpretation
      ...
      // Parse string time format HH:MM
    ...
    // Calculate proper start_date and end_date for visibility window
    let seriesStartDate, seriesEndDate;
    
    if (recurrenceData.enabled) {
      // For recurring meetings: start_date = series start, end_date = series end
      ...
      // For non-recurring meetings: start_date = meeting start, end_date = meeting end
      seriesStartDate = startDateTime.toISOString();
    ...
    // CRITICAL FIX: Send datetime strings that preserve the intended time
    const apiData = {
      ...
      // FIXED: Meeting timing - send as intended local time
      Started_At: startDateTime.toISOString(),
      Ended_At: endDateTime.toISOString(),
      start_time: startDateTime.toISOString(),
      end_time: endDateTime.toISOString(),
      
      // Visibility window for recurring meetings  
      ...
      // Enhanced recurrence data structure
      recurrence: {
